...
/**
 * Binds [BloominaRootService] once and keeps the connection alive for the
...
class RootIpc(private val context: Context) {
    ...
    suspend fun connect(): Boolean = suspendCancellableCoroutine { cont ->
        ...
        connection = conn
        val intent = Intent(context, BloominaRootService::class.java)
        LibsuRootService.bind(intent, conn)
        cont.invokeOnCancellation { disconnect() }
    }

    fun disconnect() {
        connection?.let { LibsuRootService.stop(Intent(context, BloominaRootService::class.java)) }
        connection = null
        worker = null
    }
